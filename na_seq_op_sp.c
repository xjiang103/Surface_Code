...
int main(int argc,char **args){
  ...
  PetscInt n_atoms,i,n_levels,n_seqgroups,max_seqgroupsize,val_init,pos1,pos2,dmpos=0,dmstdpos=0;
  PetscScalar tmp_scalar = 1.0,b_field,b_dr,b_0r,b_1r,gamma_r,valpar,diagsum;
  PetscInt steps_max;
  qvec dm,dmstd;
  qsystem qsys,qsysstd,qsys32;
  PetscReal dt,time_max;
  PetscReal single_qubit_gate_time=0.1,two_qubit_gate_time=0.1,var,fidelity,delta_in=0.0,deltat=0.1 ;
  circuit circ;
  ...
  PetscOptionsGetReal(NULL,NULL,"-deltat",&deltat,NULL);
	PetscOptionsGetInt(NULL,NULL,"-dmpos",&dmpos,NULL);
  PetscOptionsGetInt(NULL,NULL,"-dmstdpos",&dmstdpos,NULL);
  ...
  for(i=0;i<n_seqgroups;i++){
    ...
    pulse_params[i].stime = i*10*deltat;
    
    pulse_params[i].omega = 17.0*(2*3.1416); //MHz 2pi?
  	pulse_params[i].deltat = deltat; //us
  	pulse_params[i].delta = delta_in*17.0*(2*3.1416); //MHz 2pi?
  }
  //branching ratios
