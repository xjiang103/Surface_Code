...
int main(int argc,char **args){
  ...
  pulse_params.delta = 23.0*(2*3.1416); //MHz 2pi?
...
//		printf("\n");
 ...
 //start in the |10><10| state
    ...
    for(int l_01=0;l_01<2;l_01++){ //atom 0, 1st element
      for(int l_10=0;l_10<2;l_10++){ //atom 1, 0th element
        for(int l_11=0;l_11<2;l_11++){ //atom 1, 1st element
              ...
              pos1=l_00*2+l_10*1;
							pos2=l_01*2+l_11*1;
              add_to_qvec(dm_arp,valpar,pos1,pos2);
            }
          }
        }
      }
  ...
  fp_read=fopen("init_11.txt","r");
	...
	PetscScalar final_state_conj[16];
  ...
  for(int i=0;i<4;i++){
  	for (int j=0;j<4;j++){
  		int pos=i*4+j*1;
      tmp_val = final_state[pos]*final_state_conj[pos];
      printf("Added dm[%d][%d]: %f %f\n",i,j,tmp_val);
...
//		printf("\n");
	...
	val=1;
	get_fidelity_qvec(dm_arp,dm_std,&fidelity,&var);
