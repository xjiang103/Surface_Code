...
int main(int argc,char **args){
  ...
  PetscScalar tmp_scalar = 1.0,b_field,b_dr,b_0r,b_1r,gamma_r,valpar,valseq;
  PetscInt steps_max;
  ...
  create_qvec_sys(qsyspar8,&(dmpar8));
  create_qvec_sys(qsysseq8,&(dmseq8));
	...
	for(int i=0;i<8;i++){
		...
		//printf("%lf + %lf j\n", (double)PetscRealPart(init_state_conj[i]), (double)PetscImaginaryPart(init_state_conj[i]));
	}
...
//	add_to_qvec(dmstd,1.0,5,5); //start in the |101><101| state
  ...
  add_gate_to_circuit_sys(&circ,0.0,CZ,2,0);
  add_gate_to_circuit_sys(&circ,0.5,CZ,2,1);
