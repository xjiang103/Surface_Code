...
##f=open("quac_mat_0719.txt",'r')
##
##quac_mat=np.zeros((9,9),dtype=complex)
##for i in range(19):
##    r=f.readline()
...
##f.close()
##    
##quac_m=Qobj(quac_mat,dims=[[3,3],[3,3]])
##
...
b_field = 200.0*(2*np.pi)

omega_amp=17.0*(2*np.pi)

delta_amp=23.27121582*(2*np.pi)
...
def qutip_phase(params,qvec):
    ...
    cz_arp = Qobj(np.diag([1,-1,-1,-1])
                   ,dims=[[2,2],[2,2]])
    #Get two hadamard state
    state = tensor(snot(),snot())*tensor(basis(2,1),basis(2,1))

    #Apply phase gates with parameters that we are optimizing
    state = tensor(phasegate(params[0]),qeye(2))*state
    state = tensor(qeye(2),phasegate(params[1]))*state
...
H_leak=(-1/1)*(1j)*b_dr*gamma_r*(tensor(r_r,qeye(3))+
                               tensor(qeye(3),r_r))                               

H1=(tensor(hct,qeye(3))+
    tensor(qeye(3),hct))

H2=(tensor(r_r,qeye(3))+
    tensor(qeye(3),r_r))
...
enum2=[0,1,3,4]
qvecmat=np.zeros((4),dtype=complex)
for i in range(4):
    qvecmat[i]=res[enum2[i]]
qvec=Qobj(qvecmat,dims=[[2,2],[1,1]])

print(qvec)
print(qvec.norm())

res = minimize(qutip_phase,[0,0],method="COBYLA",args=(qvec))
fid = 1-res.fun
print("Fidelity=",fid)
print("Phase: ",res.x)
##enum2=[
