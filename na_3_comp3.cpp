...
vec_op *atomspar,*atomsseq,*atomsstd;
...
int main(int argc,char **args){
  ...
  qsystem qsyspar,qsysseq,qsysstd;
  ...
  atomsseq = malloc(n_atoms*sizeof(vec_op));
	atomsstd = malloc(n_atoms*sizeof(vec_op));
  ...
  for(i=0;i<n_atoms;i++){
    ...
    create_vec_op_sys(qsysstd,n_levels,&(atomsseq[i]));
  }
...
*/
	...
	add_to_qvec(dmstd,1.0,42,42); //start in the |111><11| state
  ...
  apply_circuit_to_sys(qsysstd,&circ,0.0);
  //Do the timestepping
  ...
  get_fidelity_qvec(dmpar,dmstd,&fidelity,&var);
  printf("fidelity between par and std is %lf\n",fidelity);
