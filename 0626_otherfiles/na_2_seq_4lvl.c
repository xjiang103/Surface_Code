...
PetscErrorCode ts_monitor(TS ts,PetscInt step,PetscReal time,Vec rho_data,void *ctx){
  ...
  get_expectation_value_qvec(dm_dummy,&trace_val2,4,atoms[0][r],atoms[0][r],atoms[1][one],atoms[1][one]);
  get_expectation_value_qvec(dm_dummy,&trace_val3,4,atoms[0][one],atoms[0][one],atoms[1][r],atoms[1][r]);
  ...
  PetscFPrintf(PETSC_COMM_WORLD,data_fp,"%d %f %f %f %f %f\n",step,time,PetscRealPart(trace_val),PetscRealPart(trace_val2),PetscRealPart(trace_val3),PetscRealPart(trace_val4));
