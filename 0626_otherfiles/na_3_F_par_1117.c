...
int main(int argc,char *args[]){
  PetscInt n_atoms,i,j,n_levels,n_seqgroups,max_seqgroupsize,val_init,pos1,pos2,dmpos,dmstdpos,flag_str;
  ...
  PetscPrintf(PETSC_COMM_WORLD,"Simulating bitstr %s with flag_str=%d, dmpos=%d and dmstdpos=%d\n",bitstr,flag_str,dmpos,dmstdpos);
  ...
  if(flag_str==0){
    ...
    printf("test ave state\n");
    add_to_qvec(dm,1.0,0,0); //start in the |00><00| state
    assemble_qvec(dm);
    for(i=0;i<n_atoms;i++){
      apply_1q_na_gate_to_qvec(dm,HADAMARD,atoms[i][zero]); //apply H to atom i
    }
      }          
