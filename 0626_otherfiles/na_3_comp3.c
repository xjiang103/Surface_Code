...
qvec dmpar_dummy,dmseq_dummy,dmseq8;
...
int main(int argc,char **args){
  ...
  for(i=0;i<n_atoms;i++){
    tmp_scalar = 1.0;
    //1.0 * omega(t) * |r><1|
    add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[0],omega,2,atomspar[i][r],atomspar[i][zero]);
    //1.0 * omega(t) * |1><r|
    add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[0],omega,2,atomspar[i][zero],atomspar[i][r]);
...
*/
	add_to_qvec(dmpar,1.0,8,8); //start in the |101><101| state
	add_to_qvec(dmseq,1.0,8,8); //start in the |101><101| state
	add_to_qvec(dmstd,1.0,2,2); //start in the |101><101| state
  assemble_qvec(dmpar);
  assemble_qvec(dmseq);
  assemble_qvec(dmstd);
