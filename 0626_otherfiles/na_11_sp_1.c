...
int main(int argc,char **args){
  ...
  PetscReal dt,time_max,val,fidelity,delta_in=0.0,deltat=0.1;
  ...
  PetscOptionsGetReal(NULL,NULL,"-delta_in",&delta_in,NULL);
  ...
  data_fp = fopen("neutral_atom_fig23.dat","w");
  ...
  //pulse_params.deltat = 0.2165; //us
  pulse_params.deltat = 0.2; //us
  //pulse_params.delta = -0.8635*17.0*(2*3.1416); //MHz 2pi?
  pulse_params.delta = -0.5*17.0*(2*3.1416); //MHz 2pi?

//  b_field = 600*(2*3.1416); //2pi? 
//  pulse_params.deltat = deltat; //us 
//  pulse_params.delta = delta_in*17.0*(2*3.1416); //MHz 2pi? 

  b_field = 60*(2*3.1416); //2pi? 
  pulse_params.deltat = deltat; //us 
  pulse_params.delta = delta_in*17.0*(2*3.1416); //MHz 2pi? 

//  b_field = 4*(2*3.1416); //2pi?
//  pulse_params.deltat = deltat; //us
//  pulse_params.delta = delta_in*17.0*(2*3.1416); //MHz 2pi?
...
//		printf("\n");
 ...
 //start in the |10><10| state
  assemble_qvec(dm);
	...
	printf("Intial dm is:\n");
	print_qvec(dm);
  time_max  = 10*pulse_params.deltat;
  dt        = 0.00001;
  steps_max = 100000;
...
PetscScalar omega(PetscReal time,void *ctx){
  ...
  p=exp(-pow((time-5*dt),2)/pow(dt,2));
