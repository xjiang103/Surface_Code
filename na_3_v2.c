...
typedef struct {
  ...
  PetscReal length,flag,stime;
} PulseParams;

int main(int argc,char **args){
  PetscInt n_atoms,i,n_levels,pos1,pos2;
  PetscScalar tmp_scalar = 1.0,b_field,b_dr,b_0r,b_1r,gamma_r,valpar,tmp_val;
  PetscInt steps_max;
  qvec dmpar,dmstd;
  ...
  //add hamiltonian terms for ancilla
  PulseParams pulse_params[3];
  for(i=0;i<3;i++){
  	pulse_params[i].omega = 17.0*2*PETSC_PI; //MHz 2pi?
    pulse_params[i].length = 0.54; //us
    pulse_params[i].delta = 23.0*2*PETSC_PI; //MHz 2pi?
    pulse_params[i].stime=0;
    pulse_params[i].flag = i;
  }
  
  tmp_scalar = 1.0;
  add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[0],omega,2,atomspar[0][r],atomspar[0][one]);
  add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[0],omega,2,atomspar[0][one],atomspar[0][r]);
  add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[0],delta,2,atomspar[0][r],atomspar[0][r]);
  //Add hamiltonian terms for data
  
  for(i=1;i<n_atoms;i++){
    tmp_scalar = 1.0;
    add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[1],omega,2,atomspar[i][r],atomspar[i][one]);
    add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[1],omega,2,atomspar[i][one],atomspar[i][r]);
    
    add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[2],omega,2,atomspar[i][r],atomspar[i][zero]);
    add_ham_term_time_dep(qsyspar,tmp_scalar,&pulse_params[2],omega,2,atomspar[i][zero],atomspar[i][r]);
  ...
  add_ham_term(qsyspar,b_field*0,4,atomspar[1][r],atomspar[1][r],atomspar[2][r],atomspar[2][r]);
...
*/
	add_to_qvec(dmpar,1.0,32,32); //start in the |101><101| state
	add_to_qvec(dmstd,1.0,0,0); //start in the |101><101| state
  assemble_qvec(dmpar);
  assemble_qvec(dmstd); 
...
PetscScalar omega(PetscReal time,void *ctx){
  ...
  PetscReal tau,t1,t2,ts,p,a,val1,val2,flg;
  PulseParams *pulse_params = (PulseParams*) ctx;   /* user-defined struct */
  ...
  if(time-ts>0 && time-ts<pulse_params->length/2){
    p =val1* (exp(-pow((time-ts-t1),4)/pow(tau,4)) - a)/(1-a);
  } else if(time-ts>pulse_params->length/2 && time-ts<pulse_params->length){
    p =val2* (exp(-pow((time-ts-t2),4)/pow(tau,4)) - a)/(1-a);
  } else{
    p = 0.0;
  }
